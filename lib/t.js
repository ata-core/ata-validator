...
function string(opts) {
  return opts === undefined ? { type: 'string' } : { type: 'string', ...opts };
}

function number(opts) {
  return opts === undefined ? { type: 'number' } : { type: 'number', ...opts };
}

function integer(opts) {
  return opts === undefined ? { type: 'integer' } : { type: 'integer', ...opts };
}
...
function array(items, opts) {
  return opts === undefined ? { type: 'array', items } : { type: 'array', items, ...opts };
}
...
function record(values, opts) {
  return opts === undefined ? { type: 'object', additionalProperties: values } : { type: 'object', additionalProperties: values, ...opts };
}

function object(properties, opts) {
  // The common case has no optional member: then `properties` is copied as it
  // is and the fresh key list is `required`. The values come from one
  // Object.values call because reading them back key by key was the larger
  // part of this function's cost. A member marked optional sends the whole
  // object down the slower path that rewrites it.
  const keys = Object.keys(properties);
  const values = Object.values(properties);
  let anyOptional = false;
  for (let i = 0; i < values.length; i++) {
    const schema = values[i];
    if (schema && schema[OPTIONAL] === true) { anyOptional = true; break; }
  }
  let out;
  if (!anyOptional) {
    out = keys.length
  ...
  return opts === undefined ? out : Object.assign(out, opts);
}
