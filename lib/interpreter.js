    ...
    if (P.hasObject && bits === T_OBJECT) {
      ...
      if (P.required !== null) {
        ...
        for (let i = 0; i < req.length; i++) {
          const key = req[i];
          if (!Object.hasOwn(data, key)) { errors.push(err('required', 'required', instancePath, schemaPath + '/required', { missingProperty: key }, `must have required property '${key}'`)); valid = false; }
        }
      }
        ...
        for (const [key, deps] of P.dependentRequired) {
          if (Object.hasOwn(data, key)) {
            for (const dep of deps) {
              if (!Object.hasOwn(data, dep)) { errors.push(err('required', 'required', instancePath, schemaPath + '/dependentRequired', { missingProperty: dep }, `must have required property '${dep}'`)); valid = false; }
            }
          }
        }
      }
