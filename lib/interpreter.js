...
class Interpreter {
  ...
  eval(P, data, base, dynScope, errors, instancePath, schemaPath, stack, sink) {
    ...
    // to first#/$defs/stuff still passes through resource `first`.
    ...
    if (P.hasString && bits === T_STRING) {
      if (P.minLength !== undefined && codePointLength(data) < P.minLength) { errors === NOERRORS || errors.push(err('minLength', 'minLength', instancePath, schemaPath + '/minLength', { limit: P.minLength }, `must NOT have fewer than ${P.minLength} characters`)); valid = false; }
      if (P.maxLength !== undefined && codePointLength(data) > P.maxLength) { errors === NOERRORS || errors.push(err('maxLength', 'maxLength', instancePath, schemaPath + '/maxLength', { limit: P.maxLength }, `must NOT have more than ${P.maxLength} characters`)); valid = false; }
