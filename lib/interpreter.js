'use strict';
const { collapseBranches } = require('./branch-collapse');
...
function typeMatches(type, d) {
  switch (type) {
    ...
    case 'number': return Number.isFinite(d);
...
class Plan {
  ...
  link(interp) {
    ...
    // Macro keywords: the schema the macro returns is applied in place, the
    // way the reference validator does it, so its errors carry the keyword's
    // path and the keyword adds an error of its own when that schema fails.
    this.macros = null;
    if (interp.keywords !== null) {
      for (const key of Object.keys(schema)) {
        ...
        this.macros.push({ keyword: key, node: child(expanded) });
      }
    }
    this.hasApplicators = this.allOf !== null || this.anyOf !== null || this.oneOf !== null || this.not !== undefined || this.if !== undefined || this.macros !== null;
    ...
    // the per-value check, built once here for the compile form, and the
    // type mask it applies to (T_ANY when the definition names no type).
    this.custom = null;
    if (interp.keywords !== null) {
      for (const key of Object.keys(schema)) {
        const def = interp.keywords[key];
        ...
        if (def.compile !== null) {
          ...
          // Errors written by the validate function land on the function
          // itself, which is where the check reads them from below.
          fn.source = validate;
        }
        if (this.custom === null) this.custom = [];
        this.custom.push({ keyword: key, fn, mask });
      }
    }
...
// Custom keyword checks for one node. The verdict variant stops at the first
// failing keyword; the collecting variant runs them all and reports each,
// taking the error objects the keyword's function left on itself when it
// wrote any (`fn.errors`, the same convention as the default validator's
// custom keywords) and a fixed message otherwise.
  ...
  for (let i = 0; i < ops.length; i++) {
    ...
    if (Array.isArray(own) && own.length > 0) {
      for (let j = 0; j < own.length; j++) {
        const e = own[j];
        errors.push(err(op.keyword, e.keyword || op.keyword,
          e.instancePath !== undefined ? instancePath + e.instancePath : instancePath,
          e.schemaPath !== undefined ? e.schemaPath : kwPath,
          e.params !== undefined ? e.params : {},
          e.message !== undefined ? e.message : `must pass "${op.keyword}" keyword validation`));
      }
    } else {
      errors.push(err(op.keyword, op.keyword, instancePath, kwPath, { keyword: op.keyword }, `must pass "${op.keyword}" keyword validation`));
    }
  }
  return valid;
}
...
class Interpreter {
  constructor(rootSchema, options) {
    ...
    // Custom keywords, already normalized by lib/keywords.js.
  ...
  eval(P, data, base, dynScope, errors, instancePath, schemaPath, stack, sink) {
    ...
    if (P.hasArray && bits === T_ARRAY) {
      ...
      if (P.contains !== undefined) {
        ...
        if (P.maxContains !== undefined && matched.length > P.maxContains) { errors === NOERRORS || errors.push(err('contains', 'contains', instancePath, schemaPath + '/contains', { minContains: minC, maxContains: P.maxContains }, `must NOT contain more than ${P.maxContains} valid item(s)`)); valid = false; }
    ...
    if (P.hasObject && bits === T_OBJECT) {
      ...
      if (P.propertyNames !== undefined) {
        for (const key of keys) {
          if (!this.eval(P.propertyNames, key, base, dynScope, errors, instancePath, schemaPath + '/propertyNames', stack, DISCARD)) valid = false;
        }
      }
      ...
      for (let k = 0; k < keys.length; k++) {
        ...
        if (!evaluated && P.additionalProperties !== undefined) {
          // `additionalProperties: false` keeps its own keyword and names the
          // property it turned away. Running it as a plain false subschema
          // reported `not` at the key's path, which drops both.
    ...
    if (P.hasApplicators) {
      ...
      if (P.anyOf !== null) {
        let any = false;
        const scratch = errors === NOERRORS ? NOERRORS : [];
        // Per-branch errors, taken as a slice of the shared scratch rather than
        // an array each, so a branch that passes costs nothing extra.
        ...
        for (let i = 0; i < P.anyOf.length; i++) {
          ...
          if (branches !== null) branches.push({ valid: ok, errors: scratch.slice(mark), title: branchTitle(P.anyOf[i]) });
          if (ok) {
            any = true;
        ...
        if (!any) {
          if (branches !== null) {
            const collapsed = collapseBranches({ keyword: 'anyOf', branchResults: branches, parentPath: instancePath, parentSchemaPath: schemaPath + '/anyOf' });
      ...
      if (P.oneOf !== null) {
        ...
        let winner = null;
        const scratch = errors === NOERRORS ? NOERRORS : [];
        // Per-branch results, so a failing oneOf collapses to the branch that
        // came closest instead of every branch's complaints at once.
        const branches = errors === NOERRORS ? null : [];
        for (let i = 0; i < P.oneOf.length; i++) {
          ...
          if (branches !== null) branches.push({ valid: ok, errors: scratch.slice(mark), title: branchTitle(P.oneOf[i]) });
          if (ok) {
            count++;
            winner = sub;
          }
        ...
        if (count === 1) { if (collect) mergeAnnotations(local, winner); }
          ...
          if (branches !== null) {
            const collapsed = collapseBranches({ keyword: 'oneOf', branchResults: branches, parentPath: instancePath, parentSchemaPath: schemaPath + '/oneOf' });
            if (collapsed !== null) errors.push(collapsed);
          }
          valid = false;
        }
...
// One branch title, for the collapsed anyOf/oneOf error. The compiled node
// keeps the schema it came from, which is where a `title` lives.
...
const { compileInterpreter } = require('./plan-compiler').install({
  Plan, NOERRORS, err, evalLeaf, evalLeafV, runCustomV, dataBits, escapePointer,
...
// For lib/plan-source.js, which emits source from the same plans and must call
// the same helpers so the two cannot answer differently.
Object.defineProperty(module.exports, '_planInternals', {
  value: { Plan, deepEqual, cpAtLeast, cpAtMost, multipleOfOk, dataBits, T_STRING, T_NUMBER, T_INTEGER, T_BOOLEAN, T_NULL, T_OBJECT, T_ARRAY, T_ANY },
  enumerable: false,
});
