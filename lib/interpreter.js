    ...
    if (P.hasApplicators) {
      ...
      if (P.oneOf !== null) {
        ...
        if (count === 1) { if (collect) mergeAnnotations(local, winner); }
          ...
          if (errors !== NOERRORS) errors.push(err('oneOf', 'oneOf', instancePath, schemaPath + '/oneOf', { passingSchemas: count === 0 ? null : passing }, 'must match exactly one schema in oneOf'));
          valid = false;
        }
      }
      if (P.not !== undefined) {
