...
class Interpreter {
  ...
  // The closure-tree compiler covers single-resource schemas without
  // unevaluated keywords; everything else stays on eval(). Compiled lazily so
  // construction stays cheap for schemas that never validate.
  _fastRoot() {
  ...
  validate(data) {
    ...
    if (fast !== null) {
      const errors = [];
      const valid = fast.c(data, errors, '', '#', []);
...
const { compileInterpreter } = require('./plan-compiler').install({
  Plan, NOERRORS, err, evalLeaf, evalLeafV, dataBits, escapePointer, mergeAnnotations,
  T_ARRAY, T_OBJECT, resolveRef, splitFragment, resolveUri,
});
