...
class Plan {
  constructor(schema, interp) {
    ...
    this.prefixItems = Array.isArray(schema.prefixItems) ? schema.prefixItems : null;
    ...
    this.properties = isObjectMap(schema.properties) ? schema.properties : null;
    this.patternProperties = null;
    if (isObjectMap(schema.patternProperties)) {
      this.patternProperties = Object.keys(schema.patternProperties).map((src) => ({ src, re: interp.pattern(src), schema: schema.patternProperties[src] }));
    }
    this.additionalProperties = schema.additionalProperties;
    this.dependentSchemas = isObjectMap(schema.dependentSchemas) ? Object.entries(schema.dependentSchemas) : null;
    this.propertyDependencies = null;
    if (isObjectMap(schema.propertyDependencies)) {
      this.propertyDependencies = Object.entries(schema.propertyDependencies).filter(([, choices]) => isObjectMap(choices));
    }
    ...
    this.allOf = Array.isArray(schema.allOf) ? schema.allOf : null;
    this.anyOf = Array.isArray(schema.anyOf) ? schema.anyOf : null;
    this.oneOf = Array.isArray(schema.oneOf) ? schema.oneOf : null;
    this.not = schema.not;
    this.if = schema.if;
    this.then = schema.then;
    this.else = schema.else;
    this.hasApplicators = this.allOf !== null || this.anyOf !== null || this.oneOf !== null || this.not !== undefined || this.if !== undefined;

    this.unevaluatedProperties = schema.unevaluatedProperties;
    this.unevaluatedItems = schema.unevaluatedItems;
...
class Interpreter {
  ...
  validate(data) {
    ...
    const valid = this.eval(this.root, data, this.state.rootBase, dynScope, errors, '', '#', [], DISCARD);
  ...
  eval(schema, data, base, dynScope, errors, instancePath, schemaPath, stack, sink) {
    ...
    if (P.ref !== null) {
      const { node, base: refBase } = resolveRef(P.ref, base, this.state);
      if (node === undefined) {
    ...
    if (P.hasObject && bits === T_OBJECT) {
      ...
      for (let k = 0; k < keys.length; k++) {
        ...
        if (props !== null && Object.prototype.hasOwnProperty.call(props, key)) {
          ...
          if (!this.eval(props[key], data[key], base, dynScope, errors, instancePath + '/' + ek, schemaPath + '/properties/' + ek, stack, DISCARD)) valid = false;
          evaluated = true;
        }
        if (patterns !== null) {
          for (let pi = 0; pi < patterns.length; pi++) {
            ...
            if (pp.re.test(key)) {
              if (!this.eval(pp.schema, data[key], base, dynScope, errors, instancePath + '/' + escapePointer(key), schemaPath + '/patternProperties/' + escapePointer(pp.src), stack, DISCARD)) valid = false;
              evaluated = true;
            }
          }
        }
