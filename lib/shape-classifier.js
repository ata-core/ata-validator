...
function isTier0Object(schema) {
  ...
  // The tier-0 validator checks presence through the per-property constraints,
  // so a required name with no entry in properties was never checked at all:
  // `{ type: 'object', required: ['id'] }` accepted `{}` until the schema moved
  // to generated code, and for good when the generator declined it.
  if (schema.required !== undefined) {
    for (const r of schema.required) {
      if (props === undefined || props === null || typeof props !== 'object' || !Object.prototype.hasOwnProperty.call(props, r)) return false;
    }
  }
