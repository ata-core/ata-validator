...
function genCodeE(schema, v, pathExpr, lines, ctx, schemaPrefix) {
  ...
  if (schema.uniqueItems) {
    ...
    const inner = isPrim
      ? `const _s${si}=new Map();for(let _i=0;_i<${v}.length;_i++){const _prev=_s${si}.get(${v}[_i]);if(_prev!==undefined){${fail('uniqueItems', 'uniqueItems', `{i:_prev,j:_i}`, `'must NOT have duplicate items (items ## '+_i+' and '+_prev+' are identical)'`)};break};_s${si}.set(${v}[_i],_i)}`
      : `const _cn${si}=function(x){if(x===null||typeof x!=='object')return typeof x+':'+x;if(Array.isArray(x))return'['+x.map(_cn${si}).join(',')+']';return'{'+Object.keys(x).sort().map(function(k){return JSON.stringify(k)+':'+_cn${si}(x[k])}).join(',')+'}'};const _s${si}=new Map();for(let _i=0;_i<${v}.length;_i++){const _k=_cn${si}(${v}[_i]);const _prev=_s${si}.get(_k);if(_prev!==undefined){${fail('uniqueItems', 'uniqueItems', `{i:_prev,j:_i}`, `'must NOT have duplicate items (items ## '+_i+' and '+_prev+' are identical)'`)};break};_s${si}.set(_k,_i)}`
    lines.push(isArr ? `{${inner}}` : `if(Array.isArray(${v})){${inner}}`)
  }
