...
function genCode(schema, v, lines, ctx, knownType) {
  ...
  if (schema.patternProperties) {
    ...
    if (schema.additionalProperties === false && schema.properties) {
      ...
      // Generate sub-schema validators for each pattern (as inline functions)
      const subFns = []
      for (let i = 0; i < ppEntries.length; i++) {
        ...
        subFns.push(`function(_ppv){${fnBody}}`)
      }
      const subFnVar = `_ppf${pi}`
      const reArrVar = `_ppr${pi}`
      // Emit the unified for..in loop
      const guard = isObj ? '' : `if(typeof ${v}==='object'&&${v}!==null&&!Array.isArray(${v}))`
      lines.push(`${guard}{const ${subFnVar}=[${subFns.join(',')}];const ${reArrVar}=[${reVars.join(',')}]`)
      lines.push(`for(const _ppk${pi} in ${v}){`)
      lines.push(`let _ppm${pi}=${allowedSet}.has(_ppk${pi})`)
      lines.push(`for(let _ppi${pi}=0;_ppi${pi}<${reArrVar}.length;_ppi${pi}++){`)
      lines.push(`if(${reArrVar}[_ppi${pi}].test(_ppk${pi})){_ppm${pi}=true;if(!${subFnVar}[_ppi${pi}](${v}[_ppk${pi}]))return false}`)
      lines.push(`}`)
      ...
      lines.push(`for(const _ppk${pi} in ${v}){`)
      lines.push(`for(let _ppi${pi}=0;_ppi${pi}<${reArrVar}.length;_ppi${pi}++){`)
      lines.push(`if(${reArrVar}[_ppi${pi}].test(_ppk${pi})&&!${subFnVar}[_ppi${pi}](${v}[_ppk${pi}]))return false`)
      lines.push(`}`)
      lines.push(`}}`)
    }
  }
...
function compileToJSCodegenWithErrors(schema, schemaMap) {
  ...
  if (typeof schema !== 'object' || schema === null) return null
  if (!codegenSafe(schema, schemaMap)) return null
  // genCodeE doesn't handle these keywords yet (Task 6) — bail to NAPI
...
function compileToJSCombined(schema, VALID_RESULT, schemaMap) {
  ...
  if (typeof schema !== 'object' || schema === null) return null
  if (!codegenSafe(schema, schemaMap)) return null
  // genCodeC doesn't handle these keywords yet (Task 6) — bail to NAPI
  if (schema.patternProperties || schema.dependentSchemas || schema.propertyNames) return null
