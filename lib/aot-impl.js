...
function toStandaloneModule(validator, opts) {
  ...
  // { positions: true }. The walker is the runtime's buildDataPositionMap,
  // embedded verbatim via toString so the two cannot drift; that is why the
  // function in lib/data-positions.js must stay self-contained.
  const positionsCore = !(opts && opts.positions) ? '' : `const _ataPosMap = ${buildDataPositionMap.toString()};
...
function validateJSON(text) {
  ...
  try { data = JSON.parse(s); } catch (e) {
    ...
    try { const m = _ataPosMap(s); if (m['']) frame = _ataFrame(m['']); } catch (_) {}
  ...
  let map;
  try { map = _ataPosMap(s); } catch (_) { return r; }
...
${parseCore}${positionsCore}${exports}`;
}
