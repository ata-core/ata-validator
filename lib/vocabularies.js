...
function stripDisabledKeywords(schema, enabled) {
  ...
  _strip(schema, disabled, new Set())
  return schema
}

function _strip(schema, disabled, seen) {
  if (typeof schema !== 'object' || schema === null) return
  if (Array.isArray(schema)) {
    for (const each of schema) _strip(each, disabled, seen)
    return
  }
  ...
  // own dialect, so this dialect's vocabularies say nothing about it.
  if (typeof schema.$schema === 'string' && seen.size > 1) return
  ...
  for (const key of objSubs) {
    ...
    if (box && typeof box === 'object' && !Array.isArray(box)) {
      for (const each of Object.values(box)) _strip(each, disabled, seen)
    }
  }
  const arrSubs = ['allOf', 'anyOf', 'oneOf', 'prefixItems']
  for (const key of arrSubs) {
    if (Array.isArray(schema[key])) {
      for (const each of schema[key]) _strip(each, disabled, seen)
    }
  }
  const singleSubs = [
    'items',
  ...
  for (const key of singleSubs) {
    ...
    if (Array.isArray(sub)) {
      for (const each of sub) _strip(each, disabled, seen)
    } else {
      _strip(sub, disabled, seen)
    }
  }
}
