...
// Which instance type each constraining keyword acts on. A keyword sitting on
// a node whose `type` excludes it is inert: `minimum` on a string can never
// fire, so the rule the author meant is absent, which is the same fail-open
// class as a typo. Only nodes with an explicit `type` are judged, since
// without one the keyword legitimately waits for matching data.
const KEYWORD_TYPES = new Map(Object.entries({
  ...
  items: ['array'], prefixItems: ['array'], additionalItems: ['array'],
  minItems: ['array'], maxItems: ['array'], uniqueItems: ['array'],
  contains: ['array'], minContains: ['array'], maxContains: ['array'], unevaluatedItems: ['array'],
...
function checkSchemaStrict(root, options) {
  ...
  function walkSchema(node, path) {
    ...
    for (const key of Object.keys(node)) {
      ...
      // An inert keyword: the node's own type excludes the one this keyword
      // acts on, so it can never fire.
      ...
      // `required` naming a property that `properties` does not define, on a
      // node that admits no other properties: no document can satisfy it, so
      // the validator built from it rejects everything, silently.
      if (key === 'required' && Array.isArray(value) && node.properties && typeof node.properties === 'object'
          && node.additionalProperties === false && !node.patternProperties) {
        for (const name of value) {
          if (typeof name === 'string' && !Object.prototype.hasOwnProperty.call(node.properties, name)) {
            problems.push({
              path: at,
