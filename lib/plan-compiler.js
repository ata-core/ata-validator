  ...
  function compileNode(ctx, node, base, chain, ann) {
    ...
    if (P.oneOf !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack, rec) => {
        const scratch = errors === NOERRORS ? NOERRORS : [];
        let count = 0;
        // Which branches passed, not just how many: `passingSchemas` is the
        // list of their indices when more than one matched, and null when none
        // did. A bare count told a caller a oneOf had gone wrong without
        ...
        for (let i = 0; i < fns.length; i++) {
          if (collect) {
            const pl = pLen(rec), n0 = rec.n, xl = xLen(rec);
            if (fns[i].c(data, scratch, instancePath, schemaPath + '/oneOf/' + i, stack, rec)) { count++; (passing || (passing = [])).push(i); }
            else undo(rec, pl, n0, xl);
          } else if (fns[i].c(data, scratch, instancePath, schemaPath + '/oneOf/' + i, stack)) { count++; (passing || (passing = [])).push(i); }
        }
        if (count !== 1) {
          if (collect) undo(rec, pl0, n00, xl0);
          if (errors !== NOERRORS) {
            if (count === 0) for (const e of scratch) errors.push(e);
            errors.push(err('oneOf', 'oneOf', instancePath, schemaPath + '/oneOf', { passingSchemas: count === 0 ? null : passing }, 'must match exactly one schema in oneOf'));
          }
          return false;
        }
        return true;
