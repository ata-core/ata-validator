...
function install(deps) {
  const { Plan, NOERRORS, err, evalLeaf, deepEqual, dataBits, escapePointer,
  ...
  function compileNode(interp, node, base, memo) {
    if (node === true) return TRUE_FN;
    if (node === false) {
      return (data, errors, instancePath, schemaPath) => {
        if (errors !== NOERRORS) errors.push(err('false schema', 'not', instancePath, schemaPath, {}, 'boolean schema is false'));
        return false;
      };
    }
    if (!(node instanceof Plan)) return TRUE_FN;
    const P = node;
    if (P.nodeBase !== null && P.nodeBase !== base) base = P.nodeBase;
    ...
    // Cycles: register a trampoline before compiling children.
    const box = { fn: null };
    const trampoline = (d, e, ip, sp, st) => box.fn(d, e, ip, sp, st);
    perBase.set(base, trampoline);
    ...
    if (P.ref !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        const ok = target(data, errors, instancePath, schemaPath + '/$ref', stack);
        stack.length -= 2;
        return ok;
      });
    }
    if (P.dynamicRef !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        stack.push(schema, data);
        const ok = target(data, errors, instancePath, schemaPath + '/$dynamicRef', stack);
        stack.length -= 2;
        return ok;
      });
    }
    ...
    if (P.prefixItems !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        for (let i = 0; i < n; i++) {
          if (!fns[i](data[i], errors, instancePath + '/' + i, schemaPath + '/prefixItems/' + i, stack)) {
            ok = false;
    ...
    if (P.items !== undefined) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        for (let i = start; i < data.length; i++) {
          if (!fn(data[i], errors, instancePath + '/' + i, schemaPath + '/items', stack)) {
            ok = false;
            if (errors === NOERRORS) return false;
          }
        }
    ...
    if (P.contains !== undefined) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        for (let i = 0; i < data.length; i++) {
          if (fn(data[i], NOERRORS, instancePath + '/' + i, schemaPath + '/contains', stack)) matched++;
        }
        let ok = true;
        if (matched < minC) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        if (pnFn !== null) {
          for (const key of keys) {
            if (!pnFn(key, errors, instancePath + '/' + escapePointer(key), schemaPath + '/propertyNames', stack)) {
              ok = false;
              if (errors === NOERRORS) return false;
            }
          }
        ...
        for (let k = 0; k < keys.length; k++) {
          ...
          if (prop !== undefined) {
            if (!prop.fn(data[key], errors, instancePath + prop.seg, schemaPath + prop.schemaSeg, stack)) {
              ok = false;
              if (errors === NOERRORS) return false;
            }
            evaluated = true;
          ...
          if (patterns !== null) {
            for (let pi = 0; pi < patterns.length; pi++) {
              ...
              if (pp.re.test(key)) {
                if (!pp.fn(data[key], errors, instancePath + '/' + escapePointer(key), schemaPath + '/patternProperties/' + escapePointer(pp.src), stack)) {
                  ok = false;
                  if (errors === NOERRORS) return false;
                }
                evaluated = true;
          ...
          if (!evaluated && apFn !== null) {
            if (!apFn(data[key], errors, instancePath + '/' + escapePointer(key), schemaPath + '/additionalProperties', stack)) {
              ok = false;
              if (errors === NOERRORS) return false;
            }
          }
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        for (const [key, fn, ek] of entries) {
          if (Object.hasOwn(data, key)) {
            if (!fn(data, errors, instancePath, schemaPath + '/dependentSchemas/' + ek, stack)) {
              ok = false;
              if (errors === NOERRORS) return false;
            }
          }
    ...
    if (P.propertyDependencies !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        for (const [key, choices] of entries) {
          ...
          if (!fn(data, errors, instancePath, branchPath, stack)) {
            ok = false;
            if (errors === NOERRORS) return false;
          }
        }
    ...
    if (P.allOf !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        for (let i = 0; i < fns.length; i++) {
          if (!fns[i](data, errors, instancePath, schemaPath + '/allOf/' + i, stack)) {
            ok = false;
            if (errors === NOERRORS) return false;
          }
        }
    ...
    if (P.anyOf !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        let any = false;
        for (let i = 0; i < fns.length; i++) {
          if (fns[i](data, scratch, instancePath, schemaPath + '/anyOf/' + i, stack)) any = true;
        }
        if (!any) {
          if (errors !== NOERRORS) {
    ...
    if (P.oneOf !== null) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        ...
        let count = 0;
        for (let i = 0; i < fns.length; i++) {
          if (fns[i](data, scratch, instancePath, schemaPath + '/oneOf/' + i, stack)) count++;
        }
        if (count !== 1) {
          if (errors !== NOERRORS) {
    ...
    if (P.not !== undefined) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        if (fn(data, NOERRORS, instancePath, schemaPath + '/not', stack)) {
    ...
    if (P.if !== undefined) {
      ...
      steps.push((data, errors, instancePath, schemaPath, stack) => {
        if (ifFn(data, NOERRORS, instancePath, schemaPath + '/if', stack)) {
          if (thenFn !== null) return thenFn(data, errors, instancePath, schemaPath + '/then', stack);
        } else if (elseFn !== null) {
          return elseFn(data, errors, instancePath, schemaPath + '/else', stack);
        }
        return true;
      });
    }
    ...
    let fn;
    if (steps.length === 0) fn = TRUE_FN;
    else if (steps.length === 1) fn = steps[0];
    else if (steps.length === 2) {
      const [a, b] = steps;
      fn = (d, e, ip, sp, st) => {
        let ok = a(d, e, ip, sp, st);
        if (!ok && e === NOERRORS) return false;
        if (!b(d, e, ip, sp, st)) ok = false;
        return ok;
      };
    } else {
      const arr = steps;
