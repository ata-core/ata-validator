...
// ata names the keyword of a false schema `not`, at the root and nested alike,
// which is what its own error codes and every consumer switch on. The
// reference names it `false schema`. The root case used to come out of the
// code generator as `false schema` by an inconsistency that has since been
// fixed, so this shim matched the reference there and not for a nested false
// schema; the rename here makes both match.
function asReferenceKeyword(e) {
  return e.keyword === 'not' && e.message === 'boolean schema is false' ? { ...e, keyword: 'false schema' } : e;
}
...
function createShaper({ Validator, rootId, rootDoc, schemas, options, allErrors, macroKeywords }) {
  ...
  return function shapeErrors(errors, data) {
    const shaped = shape(errors, data, '').map(asReferenceKeyword);
...
module.exports = { createShaper, sortLikeReference, REFERENCE_ORDER, asReferenceKeyword };
