...
function decline() { throw DECLINE; }
...
function typeSet(schema) {
  ...
  for (const name of list) {
    if (typeof name !== 'string' || !KINDS.includes(name)) decline();
    out.add(name);
  }
  if (out.size === 0) decline();
  return out;
}
...
function compileScanner(schema, options) {
  const opts = options || {};
  const ctx = { uid: 0, helpers: [], userFormats: opts.userFormats || null };
  let body;
  try {
    const out = [];
...
function emitValue(schema, ctx, out, depth) {
  ...
  if (has('const') || has('enum')) {
    ...
    if (has('enum')) {
      if (!Array.isArray(schema.enum)) decline();
      for (const v of schema.enum) listed.push(v);
    }
    for (const v of listed) if (v !== null && typeof v === 'object') decline();
  }
...
function emitString(schema, ctx, out) {
  ...
  if (schema.minLength !== undefined && typeof schema.minLength !== 'number') decline();
  if (schema.maxLength !== undefined && typeof schema.maxLength !== 'number') decline();
  const delegated = [];
...
function emitNumber(schema, ctx, out, types) {
  ...
  for (const k of ['minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum']) {
    if (schema[k] !== undefined && typeof schema[k] !== 'number') decline();
  }
  const delegated = [];
...
function emitArray(schema, ctx, out, depth) {
  ...
  if (items !== undefined && !isPlainObject(items) && typeof items !== 'boolean') decline();
...
function emitObject(schema, ctx, out, depth) {
  ...
  if (props !== undefined && !isPlainObject(props)) decline();
  const required = schema.required;
  if (required !== undefined && !Array.isArray(required)) decline();
  const ap = schema.additionalProperties;
  if (ap !== undefined && !isPlainObject(ap) && typeof ap !== 'boolean') decline();
