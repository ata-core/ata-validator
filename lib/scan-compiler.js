...
function emitObject(schema, ctx, out, depth) {
  ...
  if (useHash) {
    out.push('var _sa' + n + '=new Int32Array(' + (Math.ceil(names.length / 32) || 1) + '),_f' + n + '=0' + (counting ? ',_c' + n + '=0' : '') + ';');
  } else {
  const seenDecl = [];
  ...
  if (names.length >= HASH_DISPATCH_MIN) {
    ...
    names.forEach((nm, id) => {
      const h = spanHash(nm);
      if (table[h] !== undefined) collision = true;
      table[h] = id;
    });
    if (!collision) {
  ...
  if (hashById !== null) {
    ...
    const table = Object.create(null);
    ordered.forEach((nm, id) => { table[spanHash(nm)] = id; });
    ctx.helpers.push(table);
    ...
    out.push('var _kid' + kn + '=' + tbl + '[_h' + kn + '];');
    out.push('if(_kid' + kn + '===undefined){' + addH + '}');
    out.push('else{var _nv' + kn + '=' + nmArr + '[_kid' + kn + '];');
