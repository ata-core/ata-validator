...
function hostnameRangeSource (v, st, en) {
  return `const _st=${st},_en=${en};const _n=_en-_st;if(_n===0||_n>253)return false;let _ll=0,_pv=46;` +
...
function email(s) {
  ...
  if (at > 64) return false; // local part, 64 characters at most
  if (s.charCodeAt(0) === 34) {
    // quoted string: must close, and the quote may only end the local part
    if (at < 2 || s.charCodeAt(at - 1) !== 34) return false;
  } else {
    if (s.charCodeAt(0) === 46 || s.charCodeAt(at - 1) === 46) return false;
    let afterDot = false;
    for (let i = 0; i < at; i++) {
      const c = s.charCodeAt(i);
      if (c > 127 || EMAIL_LOCAL[c] === 0) return false;
      if (c === 46) {
        if (afterDot) return false; // a doubled dot is not a mailbox
        afterDot = true;
      } else {
        afterDot = false;
      }
...
function emailSource (v, isStr) {
  const inner =
    `const _at=${v}.lastIndexOf('@');` +
    `if(_at<=0||_at===${v}.length-1)return false;` +
    // The local half is read in place, bounded by the '@', rather than sliced
    // out: the slice was an allocation on every call, and the doubled-dot test
    // was a second pass over the same characters. Fewer emitted bytes than the
    // version it replaces, as well as fewer.
    `if(_at>64)return false;` +
    `if(${v}.charCodeAt(0)===34){if(_at<2||${v}.charCodeAt(_at-1)!==34)return false}` +
    `else{` +
      `if(${v}.charCodeAt(0)===46||${v}.charCodeAt(_at-1)===46)return false;` +
      `let _pd=false;` +
      `for(let _i=0;_i<_at;_i++){const _lc=${v}.charCodeAt(_i);if(!${EMAIL_LOCAL_SRC('_lc')})return false;if(_lc===46){if(_pd)return false;_pd=true}else{_pd=false}}` +
    `}` +
