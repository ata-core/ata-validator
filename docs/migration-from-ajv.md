...
- **Strict mode**: `strict` and `strictSchema` are enforced for the two checks that fail open: an unknown keyword (`maxLenght` would otherwise compile with the constraint simply absent) and a dangling local `$ref`. The unknown-keyword error suggests the intended spelling, a keyword the node's own `type` makes inert is reported (`minimum` on a string), and a `required` name nothing can satisfy is reported. `ata compile --strict-schema` runs the same checks at build time and refuses to emit a module from a schema that fails them, which is where a generated-schema pipeline wants them. `strictTypes`, `strictTuples` and `strictRequired` are accepted for API compatibility and not enforced as separate dials; their two useful checks live under `strict`/`strictSchema`.
