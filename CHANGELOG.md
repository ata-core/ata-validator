...
- Reading `.errors` threw a `TypeError` for an invalid `uri-template`. `new Validator({ format: 'uri-template' }).validate('http://example.com/dictionary/{term:1}/{term').errors` raised `(_e || _e).push is not a function`. The combined generator accumulates errors in `_e`, and the emitted source for that format declared its own `const _e` for the index of a closing brace, which shadowed the accumulator inside its block. It was the only emitted format source that declared that name. The verdict was always right, which is why the suite never caught it: the suite reads verdicts, and only a caller reading the errors got the throw. Present in 1.27.1 and 1.29.0. Three checks now hold the class rather than the one format: the case itself, a static check that no emitted format source declares the accumulator's name, and a sweep of the official suite reading `.errors` on every rejection, 642 schemas and 2230 cases.

### Changed

- `additionalProperties: false` no longer materialises the object's keys on every document that has nothing extra. The error has to name the offending property, so the keys have to be materialised to report one; they do not have to be materialised to find out whether there is one. The combined and error generators both called `Object.keys` and walked the array regardless, where the verdict generator has always used a bare for-in count. A for-in decides, and the keys are built only when something is actually extra. A bare count would be wrong, since an extra key present while a declared one is missing leaves the count matching, so the gate tests each key. Measured on three declared properties: the check 12.93 to 9.71 nanoseconds against the verdict generator's 8.91 floor, and the combined function 2.80x the verdict function on a valid document down to 1.50x. The request-body schema on the docs site is a strict one, so this moves the published per-request figures: accepting a body 64.8 to 41.8 nanoseconds, rejecting it 51.5 to 28.6, reading its errors 337.4 to 246.5.
- `validateJSON(text)` asks the wire scanner for its verdict, the way `isValidJSON` already did. Above the simdjson threshold it encoded the whole document to a Buffer and called the native validator instead, which measured 349 microseconds against the scanner's 191 on a 149 KB config, 16.8% of it in `utf8Write`, and the same against the published per-platform addon as against a local build. An accepted document now stops at the scanner; a rejected one goes on to the path that produces errors, skipping an encode and a native call that would only return false. Valid 349 to 177 microseconds, invalid verdict 445 to 383, invalid with frames 773 to 712. The valid row is now 0.91x a bare `JSON.parse` of the same text, because the document is never parsed.
- A rejection is validated once rather than twice, from the first rejection on. The hybrid assembly ran the verdict function and then handed the document to the error resolver, which ran the combined function over the whole document again; the combined function decides and collects in one pass. It stays lazy, since compiling it eagerly triples what a first call costs: the hybrid answers until the first rejection and the combined function from then on. Reading errors went from 3.15x the verdict-only API to 2.14x, with accepted documents unchanged. One more layer above this one still does the same thing and gets its own change.
- The position map is built for the pointers an error names instead of for every node in the document, in the runtime and in emitted modules. A rejection names two or three pointers and nothing reads any other entry, so on a 149 KB config the map was 1322 microseconds of a 2378 microsecond frame read. A subtree that cannot contain a wanted pointer is scanned to its end and never walked, and the walk stops once every wanted pointer is found: 11.5x on pointers near the start of a document and 3.7x in the worst case for the technique, one pointer at the very end. Reading an error with its frame went from 705 to 270 microseconds on a 50 KB config and from 2378 to 772 on a 149 KB one. In a module built with `{ positions: true }` the same change took framing an error from 8.02x a `JSON.parse` of the document to 1.72x, and it costs nothing to carry: 21 bytes larger raw and 361 bytes smaller gzipped. A derivation that misses a pointer must not cost a frame, so the runtime answers anything outside the set from the full map rather than reporting no position.
- The `email` format reads both halves of the address in one forward pass. `@` is not a local-part character, so in an unquoted address the first one is the separator and the local part is checked on the way to it; the check read the string backwards with `lastIndexOf` and then forwards twice, and sliced both halves out. On a 1000-user array with `format: email`, against ajv with ajv-formats at its strictest mode, five rotating rounds: ata 42 microseconds and none of twelve edge cases wrong, `mode:full` 42 microseconds and five wrong, `mode:fast` 41 and seven wrong. That row started at ata 57 against ajv 37. The others accept a doubled dot, a leading dot and a local part over the RFC's 64 characters, and reject quoted local parts and address literals.
